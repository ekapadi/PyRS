...
class TestFit:
    ...
    def test_PeakParameters_data_values(
        ...
        np.testing.assert_array_almost_equal(
            peak_params['center'].nxdata,
            params_value['Center'].astype(np.float64)
        )
        np.testing.assert_array_almost_equal(
            peak_params['height'].nxdata,
            params_value['Height'].astype(np.float64)
        )
        np.testing.assert_array_almost_equal(
            peak_params['fwhm'].nxdata,
            params_value['FWHM'].astype(np.float64)
        )
        
        # Form factor is (1.0 - Mixing)
        expected_form_factor = (1.0 - params_value['Mixing']).astype(np.float64)
        np.testing.assert_array_almost_equal(
            peak_params['form_factor'].nxdata,
    ...
    def test_BackgroundParameters_data_values(
        ...
        for param in ['A0', 'A1', 'A2']:
            ...
            np.testing.assert_array_almost_equal(
                bg_params[param].nxdata,
                params_value[param].astype(np.float64)
            )
            np.testing.assert_array_almost_equal(
                bg_params[f'{param}_errors'].nxdata,
                params_error[param].astype(np.float64)
            )
    ...
    def test_Fit_init_fields(
        ...
        fit = _Fit._init(
            logs,
            processing_description='Test description',
