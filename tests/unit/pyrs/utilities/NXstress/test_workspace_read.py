...
class TestWorkspaceRoundtrip:
    ...
    def test_workspace_roundtrip_sample_logs(self, roundtrip_nxstress):
        ...
        assert original_logs.issubset(readback_logs), \
            f"Missing logs: {original_logs - readback_logs}"
        ...
        for coord_name in HidraConstants.SAMPLE_COORDINATE_NAMES:
            if coord_name in ws_original.get_sample_log_names():
                ...
                np.testing.assert_allclose(orig_values, read_values, atol=1e-5,
                    err_msg=f"Mismatch in {coord_name} values")
        
        # Verify subruns match
        assert np.array_equal(ws_original.get_sub_runs().raw_copy(), 
                            ws_readback.get_sub_runs().raw_copy())
    ...
    def test_workspace_roundtrip_instrument(self, roundtrip_nxstress):
        ...
        if shift_original is not None:
            ...
            np.testing.assert_allclose(shift_original.center_shift_x, 
                                     shift_readback.center_shift_x, rtol=1e-6)
            np.testing.assert_allclose(shift_original.center_shift_y, 
                                     shift_readback.center_shift_y, rtol=1e-6)
            np.testing.assert_allclose(shift_original.center_shift_z, 
                                     shift_readback.center_shift_z, rtol=1e-6)
        else:
    ...
    def test_workspace_roundtrip_masks(self, roundtrip_nxstress):
        ...
        for mask_id in ws_original._mask_dict.keys():
            ...
            assert np.array_equal(mask_orig, mask_read), \
                f"Mask {mask_id} doesn't match"
    ...
    def test_workspace_roundtrip_reduced_data(self, roundtrip_nxstress):
        ...
        for mask_id in ws_original._diff_data_set.keys():
            assert mask_id in ws_readback._diff_data_set, \
                f"Mask {mask_id} missing in readback diff_data_set"
        ...
        for mask_id in ws_original._var_data_set.keys():
            assert mask_id in ws_readback._var_data_set, \
                f"Mask {mask_id} missing in readback var_data_set"
    ...
    def test_workspace_roundtrip_raw_counts(self, roundtrip_nxstress):
        ...
        for subrun in ws_original._raw_counts.keys():
            assert subrun in ws_readback._raw_counts, \
                f"Subrun {subrun} missing in readback raw_counts"
    ...
    def test_full_roundtrip(self, roundtrip_nxstress):
        ...
        for peak_orig, peak_read in zip(peaks_original, peaks_readback):
            ...
            # Verify subruns match
            assert np.array_equal(peak_orig._sub_run_array.raw_copy(), 
                                peak_read._sub_run_array.raw_copy())
...
class TestStandaloneMethods:
    ...
    def test_sampleLogsFromNexus_standalone(self):
        ...
        logs_coll['test_log'] = NXfield(np.array([10.0, 20.0, 30.0]), 
                                       local_name='test:log:pv',
                                       units='V')
    ...
    def test_masksFromNexus_standalone(self):
        ...
        # Add mask names
        mask_names = np.array([DEFAULT_TAG, 'mask_A', 'mask_B'], 
                             dtype=FIELD_DTYPE.STRING.value)
