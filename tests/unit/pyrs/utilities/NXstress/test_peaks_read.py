...
"""
import numpy as np
from nexusformat.nexus import NXreflections, NXparameters, NXfield
import pytest
        ...
        for key in original_by_key:
            ...
            for field in ['Center', 'Height', 'FWHM', 'Mixing']:
                np.testing.assert_allclose(
                    orig_eff_vals[field], 
                    recon_eff_vals[field],
                    atol=1e-5,
                    err_msg=f"Mismatch in {field} values"
                )
                np.testing.assert_allclose(
                    orig_eff_errs[field],
                    recon_eff_errs[field],
                    atol=1e-5,
                    err_msg=f"Mismatch in {field} errors"
                )
