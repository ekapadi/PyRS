...
class Gaussian(PeakParametersConverter):
    ...
    def calculate_native_parameters(self, eff_value_array, eff_error_array):
        ...
        if eff_value_array.size != eff_error_array.size:
            raise RuntimeError(
                'size of values and errors do not match: {} and {}'.format(
                    eff_value_array.size, eff_error_array.size))
        ...
        native_dtype = get_parameter_dtype(
            peak_shape=PeakShape.GAUSSIAN, background_function=bg_func)
...
class PseudoVoigt(PeakParametersConverter):
    ...
    def calculate_native_parameters(self, eff_value_array, eff_error_array):
        ...
        native_dtype = get_parameter_dtype(
            peak_shape=PeakShape.PSEUDOVOIGT, background_function=bg_func)
        native_values = np.zeros(eff_value_array.size, dtype=native_dtype)
        native_errors = np.zeros(eff_value_array.size, dtype=native_dtype)
